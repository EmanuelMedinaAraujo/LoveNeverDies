...
## Step 1: Initialize Clerk

Inspect the current directory to determine whether this is an existing
project or an empty directory.

This setup is linked to the Clerk application `app_3IEII7CYI5MAH2RIajEorZ1Fd8U`. Always
pass `--app app_3IEII7CYI5MAH2RIajEorZ1Fd8U` to `clerk init` so the project links to
the right Clerk app.

If this is an existing project, run:

```bash
clerk init --app app_3IEII7CYI5MAH2RIajEorZ1Fd8U
```
...
```bash
clerk init --framework <framework> --pm <package-manager> --app app_3IEII7CYI5MAH2RIajEorZ1Fd8U
```
...
## Step 2: Verify the Next.js matcher
...
## Step 3: Fall back to docs when init is incomplete
...
## Step 4: Ensure clear auth controls are visible
