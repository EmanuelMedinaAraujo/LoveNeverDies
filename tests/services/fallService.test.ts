...
/** Ein Server: drei Tabellen, keine Meinung. */
function server() {
  ...
  return { faelle, wraps, geraete, faelleZeilen, wrapZeilen, meldeGeraetAn }
}
...
async function angelegterFall() {
  ...
  const fall = await legeTrauerfallAn(s.faelle, eigene, GERAET, ANGABEN)
...
describe('Einen Trauerfall anlegen (§2, §3.1)', () => {
  ...
  it('weist einen leeren Namen zurück', async () => {
    ...
    await expect(
      legeTrauerfallAn(s.faelle, eigene, GERAET, { personName: '   ', sterbedatum: '2026-05-12' }),
  ...
  it('weist ein Sterbedatum zurück, das keines ist', async () => {
    ...
    for (const sterbedatum of ['12.05.2026', '2026-13-01', '2026-02-30', '']) {
      await expect(
        legeTrauerfallAn(s.faelle, eigene, GERAET, { personName: 'Hans Weber', sterbedatum }),
      ).rejects.toThrow(FallFehler)
    }
  })
})
