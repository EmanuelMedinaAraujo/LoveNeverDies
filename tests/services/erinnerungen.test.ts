...
function plan(aufgaben: Aufgabe[], jetzt: Date) {
  return planeErinnerungen(baueBaum(aufgaben), STERBEDATUM, jetzt)
}
...
describe('planeErinnerungen (§7)', () => {
  ...
  it('erinnert nicht an eine Frist ab Kenntnis, solange das Datum fehlt', () => {
    ...
    const vorlage = mitFrist(42)
    const ausschlagung =
      vorlage.katalog === null
        ? vorlage
        : { ...vorlage, katalog: { ...vorlage.katalog, fristAb: 'kenntnis' as const } }

    expect(plan([ausschlagung], morgens('2026-05-12'))).toEqual([])
  })
  ...
  it('plant ohne Sterbedatum nichts', () => {
    expect(planeErinnerungen(baueBaum([mitFrist(10)]), null, morgens('2026-05-12'))).toEqual([])
  })
})
